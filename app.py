...
def parking_data():
    ...
    try:
        ...
        ser = serial.Serial('COM12', 9600, timeout=2)  # Added timeout for robustness
        ...
        print(f"CSV Data: {csv_data}")  # Debug print
        ...
        for i, status in enumerate(slots_status):
            if status == 0 and previous_status[i] == 1:  
                entry_times[i] = time.time()  
            elif status == 1 and previous_status[i] == 0: 
                exit_times[i] = time.time()  # Record exit time
